...
const STATIC_ROUTES = [
    ...
    // Разбор набранного текста: рабочее окно, а не чтение, — но искать его
    // будут словами «проверить церковнославянский текст».
    { path: "/razbor", priority: 0.5 },
    // Церковнославянский набор: рабочие окна, а не чтение, — но искать их будут
    // словами «перекодировать UCS в юникод» и «церковнославянский шрифт не
    // отображается», и на такие вопросы в сети отвечает мало кто.
    ...
    // Перевод гражданки в церковнославянское написание живёт здесь же, и
    // ищут его словами «как написать по-церковнославянски», — оттого вес выше
    // прочих подстраниц раздела.
    { path: "/nabor/tsifir", priority: 0.5 },
    ...
    // Опыты и азбуки. Раздела в карте не было вовсе, и робот доходил до него
    // только по внутренним ссылкам — а страниц тут немного, и каждая отвечает на
    // свой вопрос, который задают поиску словами: «китайский алфавит по
    // среднекитайскому», «польская кириллица». Адреса внутри азбук (разбор
    // знака, преобразование текста) в карту не идут: за ними стоит ответ
    // движка на введённое, а не содержание, которое стоило бы искать.
    { path: "/opyty", priority: 0.6 },
    { path: "/azbuki", priority: 0.6 },
    { path: "/azbuki/chinese", priority: 0.5 },
    { path: "/azbuki/polish", priority: 0.5 },
