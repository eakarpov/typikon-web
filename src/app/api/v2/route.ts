    ...
    try {
        return respond({
            ...
            endpoints: {
                ...
                news: "/api/v2/news",
                // Единственная ручка v2, которую не запирает ключ: отсюда
                // установленная копия приложения узнаёт о новой версии, в том
                // числе о той, что чинит поломку с самим ключом.
                appVersion: "/api/v2/app/version",
                // Обратный ход из текста в службу дня и места, помеченные в
                // тексте: обоих не было, и приложение до сих пор ходило за ними
                // в первую версию API.
                textDay: "/api/v2/texts/{id}/day",
                place: "/api/v2/places/{id|alias}",
                places: "/api/v2/places?kind=&limit=&offset=",
                // Суточный круг из службы устава. Только по ключу: сборка службы
                // дороже выборки, анонимной порции на неё не положишь. День —
