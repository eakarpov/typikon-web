...
test("сырная неделя не резолвится по дате", { todo: "неделя 0 с типом Fast не совпадает с данными, где сырная седмица лежит как type Triodion, value 0" }, () => {
    // Неделя сыропустная (за 49 дней до Пасхи) попадает между двумя ветками расчёта:
    // получает type "Fast" и номер седмицы 0, которого в Триоди не существует. В базе
    // эта седмица лежит как {type: "Triodion", value: 0} (alias syrnaja, семь дней),
    // поэтому getTriodicItem ничего не находит, и на /calculator за эти даты Триодь
    // не показывается — хотя тексты есть и открываются через /triodion/{alias}.
    const cheesefareSunday = getWeekAndDay(shift(PASCHA, -49), PASCHA, PREV_PASCHA);

    assert.ok(cheesefareSunday.week >= 1, `седмица ${cheesefareSunday.week} не существует в Триоди`);
});
