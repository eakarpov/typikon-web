 ...
 * день. Заводить признак, которого он не отдаёт, значило бы просить ответственного
...
export interface ParishRule {
    ...
    priority?: number;
    /**
     * Выключено приходом. Правило не стирается, а гасится: у нашего умолчания
     * есть довод, и вернуть его потом должно быть чем.
     */
    enabled?: boolean;
    source?: string;
}
 ...
 * иначе — иначе ответственный не сможет ни проверить его, ни поспорить с ним.
 */
...
export interface Gathering {
    ...
    why: WhyStep[];
    /** Правка ответственного легла поверх уставного проекта. */
    edited?: boolean;
    /**
     * Отменено ответственным. Собрание не выбрасывается, а гасится: в подписном
