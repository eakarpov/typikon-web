...
/**
 * Вердикт словами: «сухоядение, единожды днём, два варения, до вечера».
 *
 * «ВАРЕНИЯ», А НЕ «БЛЮДА», и это слово книги, а не наше. Гл. 35 говорит «во
 * вторник, четверток и субботу, два: вечер же едино» — считает она не трапезы
 * (их считает `meals` и своими словами), а варёное, что на трапезу подаётся.
 * «Два блюда» же по-русски читается как перемены за одним столом, первое и
 * второе, — не то, о чём здесь речь. Раз книга различает дневное и вечернее,
 * пусть и счёт зовётся её словом.
 *
...
export const verdictOf = (rule: OrdoFastingRule): string => {
    ...
    if (rule.dishes === 2) parts.push("два варения");
    else if (rule.dishes === 3) parts.push("три варения");
