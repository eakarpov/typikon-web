...
export interface Allowance {
    ...
    perDay: number | null;
    /**
     * Потолок за сутки НА ОДНО УСТРОЙСТВО; null — только общий.
     *
     * Заведено для ключа, зашитого в приложение. Такой ключ не тайна — он лежит
     * внутри апк и достаётся оттуда всяким, — и защищать его нечем; но и нечего:
     * корпус под свободной лицензией и отдаётся выгрузкой целиком. Вредить им
     * можно ровно одним способом — выесть общую суточную квоту и тем положить
     * приложение у всех остальных.
     *
...
export const TIERS: Record<Tier, Allowance> = {
    ...
    // вместе расходуют сейчас две-три сотни в день. Запас тридцатикратный: живой
    // читатель в него не упрётся никогда, а один поток выкачки упрётся к обеду.
    // Общий потолок при этом остаётся: он бережёт сервер, а подушевой — читателей
    // друг от друга.
    app: {
        limit: 60, windowSeconds: 60, perDay: 500_000, perDevice: 5_000,
