...
const canonLine = (line: any) => ({
    unit: line.unit ?? null,
    /**
     * Пусто, когда ссылку разрешить не удалось: `lib/canons` подставляет туда
     * саму ссылку, чтобы страница не показала пустоту, но опознаватель
     * `he.h.m2.heHE.DefteLaoi`, напечатанный уставным кеглем, — не песнопение.
     * Клиенту честнее сказать словами, что текста в корпусе нет.
     */
    text: line.reference ? "" : (line.text ?? ""),
    /**
    ...
    /**
     * Ссылка, которую разрешить не удалось. Отдаётся, чтобы клиент мог
     * показать её мелко и как отсылку, а не как строку песнопения.
     */
...
export const placeDetail = (row: any) => ({
    ...
    // Точка берётся из `location` (GeoJSON), а у записей до миграции — из
