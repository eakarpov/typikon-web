...
// --- Чувашская Псалтирь: еврейский счёт против славянского -------------------
//
// Издание переведено с оригинала, и псалмы в нём пронумерованы по-еврейски.
// Расхождение не сквозное, а из четырёх мест — двух слияний и двух делений, —
// и проверять надо именно их края: внутри диапазонов сдвиг постоянный, а на
// стыках он меняется, и ошибка там уводит чтение в соседний псалом.
test("чувашская Псалтирь приводится к славянскому счёту", () => {
    const rules = mappingsFor("cv-ipb");
    const at = (chapter: number, verse: number) => {
        const ref = toCanonRef(rules, "psaltir", chapter, verse);
        return `${ref.chapter}:${ref.verse}`;
    };

    // до первого слияния номер псалма общий, но надписание у славянского —
    // стих, а у чувашского заголовок, и глава идёт на стих позади
    assert.equal(at(3, 1), "3:2");
    assert.equal(at(3, 8), "3:9");
    assert.equal(at(1, 1), "1:1");        // без надписания сдвига нет
    assert.equal(at(9, 20), "9:20");

    // Семьдесят слили 9-й с 10-м: еврейский 10-й идёт продолжением славянского 9-го
    assert.equal(at(10, 1), "9:21");
    assert.equal(at(10, 18), "9:38");

    // и дальше, до 113-го, еврейский счёт на псалом впереди
    assert.equal(at(11, 1), "10:1");
    assert.equal(at(113, 9), "112:9");

    // оба сдвига разом: номер псалма назад и надписание сверху
    assert.equal(at(12, 1), "11:2");
    assert.equal(at(51, 1), "50:3");

    // слили 114-й со 115-м: оба легли в славянский 113-й, 8 стихов и 18
    assert.equal(at(114, 1), "113:1");
    assert.equal(at(115, 1), "113:9");
    assert.equal(at(115, 18), "113:26");

    // разделили 116-й надвое: славянские 114-й (9 стихов) и 115-й (10)
    assert.equal(at(116, 9), "114:9");
    assert.equal(at(116, 10), "115:1");
    assert.equal(at(116, 19), "115:10");

    // после деления отставание набегает ещё раз
    assert.equal(at(117, 1), "116:1");
    assert.equal(at(146, 10), "145:10");

    // разделили 147-й: славянские 146-й (11 стихов) и 147-й (9)
    assert.equal(at(147, 11), "146:11");
    assert.equal(at(147, 12), "147:1");

    // и со 148-го счёт сходится обратно
    assert.equal(at(148, 1), "148:1");
    assert.equal(at(151, 1), "151:1");
});

test("оба чувашских издания приводятся одинаково", () => {
    const plain = mappingsFor("cv-ipb");
    const church = mappingsFor("cv-ipb-cu");
    assert.equal(plain.length, church.length);
    for (const chapter of [10, 11, 114, 115, 116, 147, 148]) {
        assert.deepEqual(
            toCanonRef(plain, "psaltir", chapter, 1),
            toCanonRef(church, "psaltir", chapter, 1),
            `псалом ${chapter} должен ложиться одинаково в обоих начертаниях`,
        );
    }
});
