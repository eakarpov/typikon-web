...
export const CacheTag = {
    ...
    BIBLE: "bible",
    // Реестр памятей приезжает выгрузкой из корпуса и меняется реже всего
    // прочего: лишь когда разбор устава научился новому.
    MEMORIES: "memories",
    // Каталог храмов и словарь посвящений: и то, и другое приезжает скриптами
    // (temples:import-*, temples:dedications), а не правкой в админке.
    TEMPLES: "temples",
    // Приходское расписание: ответ движка на дату кэшируется надолго (устав
    ...
    CITATIONS: "citations",
    // Ответ движка устава на дату: что за день, чем служится, что положено на
    // трапезу. Отдельно от PARISH нарочно: тот сбрасывается правкой правила
    // прихода и публикацией расписания, к уставу отношения не имеющими, а
    // обратное — уронить приходской проект, сбросив трапезу, — того хуже.
