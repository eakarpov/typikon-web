...
/**
 * Отделяет от леммы приставшую частицу: «ви́дѣти+же», «боя́тися+бо», «что+во».
 *
 * Плюс в словаре — не знак слова, а граница морфем: к слову приписана энклитика
 * («же», «бо», «ти», «ми», «ли») или предлог. Склоняется и спрягается при этом
 * ГОЛОВА, а хвост остаётся как есть — во всех ста пятидесяти восьми таких
 * леммах, включая «и́же+въ» и «моли́тися+бы+бо».
 *
 * Не отделив его, порождение получало лемму, у которой в конце стоит частица, а
 * не окончание схемы. `cut` и `cutSuffix` на несовпадении возвращают лемму
 * целиком — и к целому инфинитиву приписывалось окончание: «ви́дѣти+жиши»
 * вместо «ви́дишиже». Целая парадигма выходила вздором, и молча.
 *
 * Хвост возвращается без плюса: выписанные в словаре формы пишутся слитно
 * («бла́гоже», «ви́ждуже»), и порождённые должны выглядеть так же.
 */
export const splitClitic = (lemma: string): { head: string; clitic: string } => {
    const at = lemma.indexOf("+");
    if (at < 0) return { head: lemma, clitic: "" };

    return {
        head: lemma.slice(0, at),
        // Плюсов бывает и два («моли́тися+бы+бо»); хвост берём целиком, убрав их.
        clitic: lemma.slice(at + 1).split("+").join(""),
    };
};
