...
// церковнославянского языка: обе пересчитываются редко и обе едут своим релизом
...
//
// Ключ у них, вопреки прежней записи здесь, НЕ общий: lexems.search приведено к
// гражданке (ѡ→о, ѣ→е, і→и, без конечного ера), а accentKey держит ЦС-графику, и
// расходятся они у 24 633 лексем из 33 985. Словарь ударений сходится с лексемами не
// по этому полю, а по accentKey от каждой словарной формы (см. load-accents.ts).
//
