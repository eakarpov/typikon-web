...
test("слово с выносной буквой не рвётся надвое", () => {
    // До правки «Гдⷭ҇ь» распадалось на «Гд» и «҇ь», и первый обломок уходил в
    // словарь ударений отдельным словом. В собрании таких слов 14 408.
    const words = "Гдⷭ҇ь помилꙋй влⷣка".match(new RegExp(WORD_PATTERN.source, "gu")) ?? [];
    assert.deepEqual(words, ["Гдⷭ҇ь", "помилꙋй", "влⷣка"]);
    // И сокращением его делает выносная, даже когда титла нет.
    assert.equal(isAbbreviated("влⷣка"), true);
});
