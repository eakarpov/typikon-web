...
test("шрифтовые коды сводятся при индексации", () => {
    // «Маргарит» набран частными кодами шрифта: «ять короткая» вместо ѣ. Без
    // сведения такое слово не находится ни по «мнозем», ни по «мнѡѕѣмъ».
    const fields = buildSearchFields({ content: "мнѡѕ\uE0ECмъ" });
    assert.equal(fields.searchContent, "мноземъ");
    assert.equal(normalizeQuery("мнозем"), "мнозем");
});

test("уставные начертания сводятся к гражданским", () => {
    // «Маргарит» набран уставом: т трёхногое, д долгоногое, с широкое. До этого
    // «пасᲅырь» не сводился к «пастырь» и не находился ничем; в собрании таких
    // знаков 33 917 в 21 тексте.
    const fields = buildSearchFields({ content: "пасᲅырь ᲁобрый ᲃлово" });
    assert.equal(fields.searchContent, "пастырь добрыи слово");
    assert.equal(normalizeQuery("Пастырь"), "пастырь");
});

test("выносные буквы отбрасываются, а не опускаются в строку", () => {
    // «гдⷭ҇ь» с опущенной выносной дало бы «гдсь», тогда как в «господь» с стоит
    // перед д. Того же держится cslav.py в корпусе. До правки выносная оставалась
    // в поисковой копии, и такое слово не находилось ничем: 14 408 слов собрания.
    assert.equal(buildSearchFields({ content: "Гдⷭ҇ь гдⷭ҇и" }).searchContent, "гдь гди");
});

test("ук, набранный обычными о+у, сводится в начале слова", () => {
    assert.equal(buildSearchFields({ content: "оу҆слы́ши мѧ̀" }).searchContent, "услыши мя");
    // В середине слова «оу» — не ук: «поустынный» трогать нельзя.
    assert.equal(buildSearchFields({ content: "поустыни" }).searchContent, "поустыни");
});
