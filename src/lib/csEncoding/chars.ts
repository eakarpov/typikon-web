...
/**
 * Частная область: соглашение Славянской компьютерной инициативы.
 *
 * Эти коды — не мусор и не след перекодировки. Церковнославянские шрифты
 * семейства Ponomar (sci.ponomar.net) размещают в частной области то, чего
 * в юникоде нет отдельным знаком: сложенные сочетания звательца с ударением,
 * выносную букву вместе с покрытием, варианты начертаний. Значения взяты
 * из имён глифов самих шрифтов (PonomarUnicode, Acathist, Shafarik,
 * Pochaevsk), а не выведены предположительно.
 *
 * Значения сверены с Мономахом — шрифтом, которым набран «Маргарит»: начертание
 * частного кода сличалось с начертанием заявленного юникодного соответствия и,
 * для меры, с заведомо чужим. Совпадение начертаний подтвердило выносные с
 * покрытием (0.89–0.91 против 0.75–0.83 у чужого), связку двух выносных (точное
 * совпадение) и варианты букв (0.83 против 0.24). Сочетания звательца с
 * ударением начертанием не различаются и приняты по именам глифов.
 *
 * Знать их нужно по двум причинам. Во-первых, чтобы не называть их
 * бессмысленными: набор с ними правилен и в своём шрифте читается. Во-вторых,
 * у большинства из них есть юникодное соответствие в два знака, и текст,
 * набранный частными кодами, не сводится нашей нормализацией: «ять короткая»
 * не складывается с «ѣ», а значит и с «е».
 */
const SCI_PUA: Array<[number, string, string]> = [
    // Звательце и его сочетания с ударением. В юникоде выражаются двумя
    // знаками: U+0486, U+0486 U+0301, U+0486 U+0311.
    [0xe000, "звательце (прописное начертание)", "юникодное соответствие — U+0486"],
    [0xe001, "звательце с оксией", "юникодное соответствие — U+0486 U+0301"],
    [0xe002, "звательце с оксией (прописное начертание)", "юникодное соответствие — U+0486 U+0301"],
    [0xe005, "звательце с каморой", "юникодное соответствие — U+0486 U+0311"],
    [0xe01b, "титло (вариант начертания)", "юникодное соответствие — U+0483"],
    // Варианты начертаний букв. Юникодного соответствия у начертания нет,
    // но буква за ним стоит обычная, и при сведении написаний это важно.
    [0xe0e2, "ук короткий", "вариант начертания ꙋ (U+A64B); в Мономахе глифа нет"],
    [0xe0e4, "ук короткий (второй вариант)", "вариант начертания ꙋ (U+A64B)"],
    [0xe0ea, "ц усечённое", "вариант начертания ц; в Мономахе глифа нет"],
    [0xe0ec, "ять короткая", "вариант начертания ѣ (U+0463)"],
    [0xe0ef, "у (вариант начертания)", "вариант начертания у; в Мономахе глифа нет"],
    // Выносная буква вместе с покрытием, сложенные в один знак. В юникоде —
    // два знака: выносная из блока U+2DE0 и покрытие U+0487.
    [0xf4e1, "выносная в с покрытием", "юникодное соответствие — U+2DE1 U+0487"],
    [0xf4e2, "выносная г с покрытием", "юникодное соответствие — U+2DE2 U+0487"],
    [0xf4e4, "выносная ж с покрытием", "юникодное соответствие — U+2DE4 U+0487"],
    [0xf4e5, "выносная з с покрытием", "юникодное соответствие — U+2DE5 U+0487"],
    [0xf4e6, "выносная к с покрытием", "юникодное соответствие — U+2DE6 U+0487"],
    [0xf4e9, "выносная н с покрытием", "юникодное соответствие — U+2DE9 U+0487"],
    [0xf4ea, "выносная о с покрытием", "юникодное соответствие — U+2DEA U+0487"],
    [0xf4ed, "выносная с с покрытием", "юникодное соответствие — U+2DED U+0487"],
    [0xf4f0, "выносная ц с покрытием", "юникодное соответствие — U+2DF0 U+0487"],
    [0xf4f1, "выносная ч с покрытием", "юникодное соответствие — U+2DF1 U+0487"],
    [0xf0023, "выносные д и і, связанные вместе", "юникодное соответствие — U+2DE3 U+200D U+A675"],
    // Опознан не по шрифту, а по месту: все 66 вхождений в «Маргарите» стоят
    // сразу после «ра», то есть закрывают сокращение «ра́ди». Глифа нет ни в
    // одном доступном шрифте, включая Мономах, которым текст набран, — читатель
    // видит заполнитель.
    [0xe086, "знак сокращения «ради»", "опознан по месту: всегда после «ра»; глифа нет ни в одном доступном шрифте"],
];

for (const [cp, name, note] of SCI_PUA) {
    CS_CHARS[cp] = { cp, name, klass: "private", note };
}
