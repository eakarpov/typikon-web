...
export interface Saint {
    ...
    /**
     * Адрес лица по спеке корпуса (typikon-rules, spec/saint.md):
     * `saint:ioannes/-/damascus/777/-`. Им на святого ссылаются извне, не
     * имея ни нашей базы, ни нашего сайта.
     *
     * НЕ ТО ЖЕ, ЧТО slug. Слуг — адрес СТРАНИЦЫ, и он обещание: назначен
     * однажды и не меняется. URI — адрес ЛИЦА, и он вычисляется из имени,
     * чина и года: докрутили грамматику или словарь — пересчитается.
     * Прежние значения не теряются, они уходят в previousUris, и ссылка,
     * ушедшая в чужую статью, остаётся разрешимой.
     *
     * Ставит их typikon-rules/scripts/push_saint_uris.py — там же, где
     * живут грамматика и словари. Второй реализации нет нарочно.
     */
    uri?: string | null;
    /** Адреса, которые запись носила прежде. Отсюда ничего не удаляется. */
    previousUris?: string[];
    /** Версия грамматики, которой выдан текущий uri. */
    uriSpec?: string | null;
