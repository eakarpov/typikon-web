...
// ЧИСТЫЕ ПРАВИЛА ЗАЯВКИ на приём записок: коды, адрес открытой страницы и
...
/**
 * Код подтверждения для письма. Короткий: его оставляют в ответе руками.
 *
 * Не ключ и не тайна: им ничего не открыть, он только связывает ответное письмо
 * с заявкой. Слово «знак» тут было взято у приходских заявок, где им метят сайт
 * храма, — но получатель письма ни про какие знаки не знает, и в письме это
 * просто код подтверждения.
 */
export const newToken = () => `typikon-${randomBytes(5).toString("hex")}`;

/** Код-приглашение. Длиннее кода подтверждения: по нему открывается приём, и его подбирают. */
