...
// потянула бы за собою счёт, отчисления и спор о том, чья это была требa. Оплат
// у нас нет и не будет, а потому и приход тут ни при чём: записку принимает
// СВЯЩЕННИК, и отвечает за неё он сам.
//
// Оттого прихожанин приходит по коду-приглашению, какой священник раздаёт сам
// (ссылка, знак на стенде), а не ищет храм в справочнике. Кто готов принимать
// от всех — открывает страницу, и она попадает в общий список.
//
...
//
//   ОТВЕТНОЕ ПИСЬМО на адрес в домене той же епархии. Мы шлём письмо со знаком,
//   человек отвечает, модератор сверяет знак глазами.
//
...
export type ClaimStatus =
    ...
    | "pending"
    /** письмо со знаком отправлено, ждём ответа */
    | "letter-sent"
    /** ответ пришёл и знак сошёлся — осталось решить */
    | "verified"
    | "approved"
    | "rejected";
...
export interface CommemoratorClaim {
    ...
    dioceseUrl: string;
    /** Адрес, на который уйдёт письмо со знаком. */
    email: string;
    /** Не обязателен: разговор остаётся запасным путём. */
    phone?: string | null;
...
/** Ответ пришёл и знак сошёлся — сверял человек, не машина. */
