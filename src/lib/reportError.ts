...
//
// Без маркера "server-only", хотя место ему здесь по смыслу: этот пакет в проекте
// не установлен, его подменяет сборщик Next, и вне сборки — под tsx в тестах —
// он не резолвится вовсе. С маркером всякий тест, чей модуль дотягивается сюда
// (rulesDb → chants, citations, incipits, razbor), падал на импорте. Взамен —
// уговор: секретов в этом файле не держим, и когда появится Sentry, его DSN
