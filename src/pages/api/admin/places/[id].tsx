...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === 'POST') {
        ...
        try {
            ...
            const current = await places.findOne({ "_id": new ObjectId(id) }, { projection: { name: 1, names: 1, locationSource: 1 } });
            // Имя, поправленное редактором, — его: обогащение и выдача адресов перезаписывают
            // только имена импорта (nameSource openbible/wikidata), см. @/lib/places/schema.
            const renamed = typeof data.name === "string" && data.name !== current?.name;
