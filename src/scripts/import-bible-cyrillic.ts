...
const RU_BOOKS: Array<{ name: string; slug: string }> = [
    ...
    // ЭТО СЛАВЯНСКАЯ ВТОРАЯ ЕЗДРЫ, А НЕ ТРЕТЬЯ. Румынская печатает её «КА́Р̾ТѢ
    // А҆̀ ТРЕ́ЇА А҆̀ ЛꙊ́Й Е҆́З̾Д̾РА» — третьей, — но считает Ездры по-западному:
    // Неемия у неё «А ДОУА Ездра», вторая. Значит её третья — это Ἔσδρας Α
    // Семидесяти, то есть наша вторая. Прежде слуг был взят с печатного титула
    // буквально, и книга легла в `3-ezdry`: у румынской колонки не стало 2-й
    // Ездры, а 3-я заполнилась чужим текстом. Сверено по тексту (обе начинаются
    // пасхой Иосии) и по главам: семь из девяти сходятся стих в стих.
    { name: "2-я Ездры", slug: "2-ezdry" },
