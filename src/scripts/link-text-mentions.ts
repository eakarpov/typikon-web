...
// Память праздника, а не человека. Имена, вытащенные из "Рождество Господа Бога и Спаса
// нашего Иисуса Христа", ловят "Христа/Спаса/Господа" в любом пасхальном слове.
const FEAST = /рождество|пасх|вознесение|преображение|успение|благовещение|введение|покров|сретение|воздвижение|обрезание|суббота|неделя|пятидесятниц|происхождение|положение риз|нерукотворен|знамение|икон/i;

// Память группы ("Дада, Максим и Квинтилиан", "Феона, Симеон и Форвин"): каждое имя
// в ней встречается у десятка других святых, приписать упоминание кому-то одному нельзя.
const GROUP = /,\s*\p{Lu}[\p{Ll}́]+\s+и\s+\p{Lu}|\sи\s+\p{Lu}[\p{Ll}́]+\s*$|три\s|семь\s|\d/u;
...
const parseMemoTitles = (titles: string[]) => {
    ...
    for (const title of titles) {
        if (COLLECTIVE.test(title) || FEAST.test(title) || GROUP.test(title)) continue;
