import { SITE_HOST } from "@/utils/site";
// Что проект отдаёт наружу выгрузкой и на каких условиях.
//
// Корпус под CC BY 4.0, но взять его целиком до сих пор было неоткуда: постранично
// мы просим не ходить (см. robots.ts), а публичный API отмерен тридцатью запросами в
// минуту — три тысячи текстов и сто пятьдесят тысяч стихов так забираются сутками.
// Выгрузка закрывает это: один архив, честные условия, контрольные суммы.
//
// ГЛАВНОЕ ЗДЕСЬ — НЕ ФОРМАТ, А РАЗБОР ПО ПРАВАМ. В базе рядом лежит наше, чужое и
// наше-под-чужой-лицензией, и одним куском это выложить нельзя: слои описаны в
// LICENSE-CORPUS.md, и здесь они выражены кодом. Поэтому каждая коллекция базы
// обязана быть либо в каком-то слое, либо в списке исключённых с причиной —
// сборка падает, если появилась коллекция, о которой никто ничего не сказал.
// Иначе первая же новая таблица уехала бы наружу молча.

export interface DumpLicense {
    id: string;
    name: string;
    url: string;
    /**
     * Файл с полным текстом лицензии в каталоге licenses/. Нужен там, где ссылки
     * мало: GPL-3.0 требует передавать копию лицензии вместе с работой (§4).
     */
    file?: string;
}

export interface DumpCollection {
    /** Коллекция Mongo. У производных выгрузок — null, их собирает сам скрипт. */
    source: string | null;
    /** Имя файла без расширения: <file>.jsonl.gz либо <file>.csv.gz */
    file: string;
    /**
     * Формат. JSON Lines по умолчанию: он держит любую запись, включая вложенную.
     * CSV заводится там, где строки ПЛОСКИЕ и таблицу открывают не программой, а
     * глазами, — сейчас это одно согласование нумераций. Колонки перечисляются
     * явно: порядок столбцов — часть договора, и выводить его из первой записи
     * значило бы менять файл от того, какая запись легла первой.
     */
    format?: "jsonl" | "csv";
    columns?: string[];
    /**
     * Имя файла, чьи данные этот повторяет в другом формате. Нужно не для порядка,
     * а для честности чисел: в итогах такой файл записями НЕ считается, иначе
     * согласование нумераций легло бы в сумму дважды и слой вырос бы на 192 106
     * несуществующих записей.
     */
    sameAs?: string;
    title: string;
    /** Поля, которые наружу не идут: поле -> почему. Причина уходит в манифест. */
    drop?: Record<string, string>;
    /** Издание Библии, которым ограничена выгрузка (код из bible_editions). */
    edition?: string;
    /** Половина Библии: у греческого издания у них разное происхождение. */
    testament?: "ot" | "nt";
    /**
     * Своя лицензия на ОТДЕЛЬНЫЙ файл, когда она не та, что у слоя. Заведено ради
     * греческого Ветхого Завета: оцифровка чужая и под GPL-3.0, а всё остальное в
     * слое — наше под CC BY. Городить ради одного файла отдельный слой значило бы
     * прятать различие в устройстве каталогов вместо того, чтобы его назвать.
     */
    license?: DumpLicense;
    attribution?: string;
    note?: string;
}

/** Чего в слое НЕТ и где это взять самому. */
export interface DumpPointer {
    what: string;
    why: string;
    where: string;
}

export interface DumpLayer {
    /** Каталог внутри выгрузки. */
    id: string;
    title: string;
    license: DumpLicense;
    /** Как на слой ссылаться. */
    attribution: string;
    /** Почему условия именно такие. Уходит в README слоя и в манифест. */
    rationale: string;
    collections: DumpCollection[];
    /**
     * Чего в слое нет и где это взять. Не отговорка, а работа за читателя: он всё
     * равно пойдёт искать, и лучше дать ему адрес, чем оставить с пустым местом.
     */
    pointers?: DumpPointer[];
}

const CC_BY = {
    id: "CC-BY-4.0",
    name: "Creative Commons Attribution 4.0 International",
    url: "https://creativecommons.org/licenses/by/4.0/",
};

const GPL3 = {
    id: "GPL-3.0",
    name: "GNU General Public License v3.0",
    url: "https://www.gnu.org/licenses/gpl-3.0.html",
    file: "GPL-3.0.txt",
};

const PUBLIC_DOMAIN = {
    id: "PD",
    name: "Общественное достояние (Public Domain Mark 1.0)",
    url: "https://creativecommons.org/publicdomain/mark/1.0/",
};

const CC_BY_SA = {
    id: "CC-BY-SA-4.0",
    name: "Creative Commons Attribution-ShareAlike 4.0 International",
    url: "https://creativecommons.org/licenses/by-sa/4.0/",
};

const ODBL = {
    id: "ODbL-1.0",
    name: "Open Database License 1.0",
    url: "https://opendatacommons.org/licenses/odbl/1-0/",
};

export const CITATION = `Корпус «Уставные чтения» (${SITE_HOST}), CC BY 4.0`;

/**
 * Поля текста, которые наружу не идут, и почему. Общие для слоя corpus и слоя
 * fathers: отеческие тексты отдаются дважды (в корпусе целиком и отдельной
 * записью архива), и сниматься должно одно и то же — иначе две копии разошлись
 * бы в составе, а не только в отборе строк.
 */
export const TEXTS_DROP: Record<string, string> = {
    adminInfo: "служебные пометки набора, читателю снаружи не нужны",
    fileId: "внутренняя ссылка на файл нашего хранилища",
    searchContent: "производное от content, пересчитывается нормализацией",
    searchName: "производное от name, пересчитывается нормализацией",
};

export const LAYERS: DumpLayer[] = [
    {
        id: "corpus",
        title: "Корпус уставных чтений",
        license: CC_BY,
        attribution: CITATION,
        rationale:
            "Наборный текст памятников, ударения, разбиение, привязка чтений к дням и "
            + "слотам службы, разметка зачал и упоминаний — работа проекта. Сами памятники "
            + "(Пролог, Златоустник, Маргарит, Лествица и прочие) в общественном достоянии "
            + "по возрасту: на них не претендует никто, включая нас.",
        collections: [
            {
                source: "texts",
                file: "texts",
                title: "Тексты корпуса",
                drop: TEXTS_DROP,
            },
            { source: "books", file: "books", title: "Книги собрания" },
            { source: "months", file: "months", title: "Месяцы неподвижного круга" },
            { source: "days", file: "days", title: "Дни с расписанными чтениями" },
            { source: "weeks", file: "weeks", title: "Седмицы подвижного круга" },
            { source: "signs", file: "signs", title: "Знаки месяцеслова" },
            { source: "pericopes", file: "pericopes", title: "Зачала" },
            {
                source: "sources",
                file: "sources",
                title: "Описания рукописей и изданий",
                note: "Ссылки на сканы РГБ, НЭБ и lib-fond — сами сканы принадлежат хранилищам.",
            },
            {
                source: "places",
                file: "places",
                title: "Места",
                note: "Имена, точки и периоды сведены из OpenBible Bible Geocoding (CC BY 4.0), "
                    + "Pleiades (CC BY 3.0) и Wikidata (CC0); источник каждой записи назван в её полях source.",
            },
            {
                source: "place_relations",
                file: "place-relations",
                title: "Связи мест: преемственность, отождествления, вложенность",
                note: "Отождествления с баллами — OpenBible (CC BY 4.0); преемственность — Wikidata (CC0).",
            },
            {
                source: "place_mentions",
                file: "place-mentions",
                title: "Упоминания мест в Писании и текстах",
                note: "Стихи источника (OpenBible, CC BY 4.0) переведены в нумерацию Елизаветинской Библии "
                    + "сверкой по тексту; status: approved — имя найдено в стихе, pending — не найдено.",
            },
            { source: "memories", file: "memories", title: "Памяти святцев" },
            { source: "memory_saint_links", file: "memory-saint-links", title: "Связи памятей со святыми" },
            { source: "akathist_saint_links", file: "akathist-saint-links", title: "Связи акафистов со святыми" },
            {
                source: null,
                file: "accents",
                title: "Словарь ударений по корпусу",
                note:
                    "Основа -> где стоит ударение и сколько раз так написано. Выводится из "
                    + "текстов этого же слоя, поэтому отдаётся вместе с ними: два миллиона "
                    + "ударных словоформ, размеченных вручную за годы набора. Только по "
                    + "нашему корпусу — словарь церковнославянского, которым дополнен "
                    + "публичный /api/v2/accents, взят со стороны и сюда не входит.",
            },
            {
                source: null,
                file: "saint-external-ids",
                title: "Соответствия святых чужим указателям",
                note:
                    "Наш слуг -> идентификатор той же памяти в чужом указателе (сейчас "
                    + "dneslov.org). Отождествление сделано нами и потому отдаётся; сами "
                    + "перенесённые оттуда поля из каталога святых сняты. В файле только "
                    + "идентификаторы, ни слова чужого содержания. Соответствия храмов "
                    + "Викиданным и OpenStreetMap отдельным файлом не выносятся: они и так "
                    + "стоят при каждом храме в слое temples.",
            },
            {
                source: "citation_stats",
                file: "citation-stats",
                title: "Свод цитируемости Писания",
                note:
                    "Сколько стихов Писания звучит за богослужением и как: порознь то, что "
                    + "поётся, и то, что читается. Считается скриптом по корпусу песнопений и "
                    + "по зачалам; в свод входит и отпечаток корпуса, по которому видно, от "
                    + "какого его состояния числа сняты.",
            },
            {
                source: "name_index",
                file: "name-index",
                title: "Указатель имён святцев",
                note:
                    "Имя -> святые с этим именем и дни их памяти. Указателя имён у святцев "
                    + "не было: он выведен разбором заголовков памятей, и у каждой связки "
                    + "стоит помета уверенности. На нём стоят «Именины» и помянник.",
            },
            {
                source: "dedications",
                file: "dedications",
                title: "Словарь посвящений храмов",
                note: "Наш разбор имён храмов: чему посвящён престол. Им размечен слой temples.",
            },
            {
                source: "saints",
                file: "saints",
                title: "Каталог святых",
                drop: {
                    imageUrl: "изображение со стороны dneslov.org, условия на него не заявлены",
                    roundelUrl: "то же",
                    externals: "ссылки на чужие карточки, перенесены как есть",
                },
                note:
                    "Наше здесь — сведение, отождествление, имена и связи. Поля, перенесённые "
                    + "из святцев dneslov.org, сняты: их владелец условий нигде не объявил.",
            },
        ],
    },
    {
        id: "fathers",
        title: "Святоотеческие и уставные чтения",
        license: CC_BY,
        attribution: CITATION,
        rationale:
            "Срез слоя corpus: святоотеческие памятники и жития, по которым уставные "
            + "чтения выкладываются отдельной записью архива, чтобы на них можно было "
            + "ссылаться своим DOI. Наше здесь то же, что в корпусе — набор, ударения, "
            + "разметка, привязка к уставу, — и идёт под той же лицензией CC BY 4.0. "
            + "Сами памятники в общественном достоянии по возрасту; сканы принадлежат "
            + "РГБ, lib-fond и НЭБ и наружу не идут — идут ссылки на них. Те же строки "
            + "есть и в corpus/texts — это объявленное устройство, а не недосмотр: "
            + "корпусная запись осталась как была опубликована, отеческая отдаёт "
            + "свой срез поверх неё.",
        collections: [
            {
                source: null,
                file: "books",
                title: "Отеческие книги собрания",
                note: "Книги из списка src/scripts/lib/fathers.ts: толкования, беседы, "
                    + "аскетика, торжественники, синаксари и жития (прологи).",
            },
            {
                source: null,
                file: "texts",
                title: "Тексты отцов и житий",
                note: "Снятые поля — те же, что у corpus/texts. У части текстов "
                    + "readiness: presence — пустая заглушка с описанием и ссылкой "
                    + "на скан; полная сводка готовности — в файле readiness.",
            },
            {
                source: null,
                file: "sources",
                title: "Описания рукописей, по которым набираются тексты слоя",
                note: "Те записи sources, чей url — префикс ссылки (link) хотя бы "
                    + "одного отеческого текста. Сами сканы принадлежат хранилищам.",
            },
            {
                source: null,
                file: "readings",
                title: "Уставная привязка: дни и слоты службы, где читается каждый текст",
                note: "Выводится из days: у какого дня, в каком слоте службы стоит "
                    + "чтение этого текста. Дни целиком остаются в слое corpus — "
                    + "связь по их _id.",
            },
            {
                source: null,
                file: "readiness",
                title: "Свод готовности текстов по книгам",
                note: "Сколько текстов каждой книги в каком статусе: ready — вычитан, "
                    + "texted — отекстован и ждёт сверки, presence — заглушка со "
                    + "ссылкой на скан; прочие статусы (correcting и др.) перечислены "
                    + "как есть, чтобы сводка сходилась с числом текстов.",
            },
        ],
    },
    {
        id: "bible",
        title: "Библия: издания, книги и согласование нумераций",
        license: CC_BY,
        attribution: CITATION,
        rationale:
            "Наружу идёт то, что сделано здесь. Согласование нумераций — главное: у стиха "
            + "две пары чисел, родная (как напечатано в издании) и каноническая (приведённая "
            + "правилами к Елизаветинской Библии), и таблица их соответствия по пяти изданиям "
            + "выведена в проекте. Она отдаётся ЦЕЛИКОМ и без текста стиха, поэтому её можно "
            + "приложить к своей копии любого из этих изданий.\n\n"
            + "Текст стихов отдаётся там, где это позволено. Румынскую кириллицу 1688 и "
            + "китайский Новый Завет 1910 набирали мы — они под лицензией слоя. Румынская "
            + "синодальная 1914 взята из Викитеки и идёт под её CC BY-SA. Греческий "
            + "Ветхий Завет (Свит) взят готовой оцифровкой под GPL-3.0 и отдаётся ПОД НЕЙ ЖЕ, "
            + "отдельным файлом со своей лицензией: пересдать чужую работу на своих условиях "
            + "мы не вправе, но и прятать её незачем — правообладатель распространение "
            + "разрешил, только на своих условиях.\n\n"
            + "Греческий Новый Завет — общественное достояние: Патриаршее издание 1904/1912 "
            + "взято у ebible.org, который отдаёт его без притязаний.\n\n"
            + "Церковнославянский и латинский остаются вне выгрузки: их оцифровки чужие и "
            + "разрешения на распространение не дают. Где взять их самому — сказано ниже.",
        collections: [
            {
                source: "bible_editions",
                file: "bible-editions",
                title: "Издания Библии",
                note: "С правилами согласования (mapping) и ссылкой на источник оцифровки.",
            },
            { source: "bible_books", file: "bible-books", title: "Книги изданий с привязкой к канону" },
            {
                source: null,
                file: "bible-concordance",
                title: "Согласование нумераций по всем шести изданиям",
                note:
                    "Стих без текста: издание, книга, глава и стих как напечатано — и "
                    + "канонический адрес, куда это легло.",
            },
            {
                source: null,
                file: "bible-concordance",
                format: "csv",
                columns: [
                    "edition", "book", "chapter", "verse",
                    "canonId", "canonChapter", "canonVerse", "canonRef", "canonSort",
                ],
                sameAs: "bible-concordance.jsonl.gz",
                title: "То же согласование таблицей (CSV)",
                note:
                    "Тот же файл, что рядом, в другом формате. Заведён нарочно: половине "
                    + "тех, кому эта таблица нужна, — филологам и переводчикам — её открывать "
                    + "в редакторе таблиц, а не разбирать программой, и JSON Lines для них "
                    + "лишняя преграда.",
            },
            {
                source: "bible_verses",
                file: "bible-verses-ro-1688",
                edition: "ro-1688",
                title: "Библия на румынской кириллице, 1688 — текст стихов",
                note: "Оцифровка выполнена в проекте по сканам издания.",
            },
            {
                source: "bible_verses",
                file: "bible-verses-zh-1910",
                edition: "zh-1910",
                title: "Китайский Новый Завет, Пекин 1910 — текст стихов",
                note: "Разбор печатного издания выполнен в проекте.",
            },
            {
                source: "bible_verses",
                file: "bible-verses-ro-1914",
                edition: "ro-1914",
                title: "Румынская синодальная 1914 (латиница) — текст стихов",
                license: CC_BY_SA,
                attribution:
                    "Текст: Biblia 1914, ro.wikisource.org/wiki/Biblia_1914 — CC BY-SA 4.0. "
                    + "Разбор на книги и согласование нумераций — проект «Уставные чтения» "
                    + `(${SITE_HOST}), CC BY 4.0`,
                note:
                    "Вычитанный людьми текст Викитеки, а не OCR. Share-alike у него свой, "
                    + "поэтому файл идёт не под лицензией слоя. Стоит подложкой к "
                    + "кириллическому изданию 1688 года, набранному в проекте.",
            },
            {
                source: "bible_verses",
                file: "bible-verses-cs-eliz",
                edition: "cs-eliz",
                title: "Елизаветинская Библия, 1751 — текст стихов",
                license: PUBLIC_DOMAIN,
                attribution:
                    "Текст: Елизаветинская Библия (ред. 1751/1900), оцифровка "
                    + "bible.by/elzs. Разбор на книги, эталон версификации и правила "
                    + "приведения остальных изданий — проект «Уставные чтения» "
                    + `(${SITE_HOST}), CC BY 4.0`,
                note:
                    "Эталон версификации всего собрания: к нему приведены адреса "
                    + "остальных изданий. Условий оцифровщик не заявляет; отдаём по тому "
                    + "же рассуждению, что и грузинскую, — верная передача текста, "
                    + "свободного от притязаний, своего права не порождает, а издание "
                    + "1751 года в общественном достоянии по возрасту бесспорно. "
                    + "Одна оговорка к полноте: Исх. 36:9–39:23 книга печатает БЕЗ "
                    + "нумерации стихов и прямо о том говорит пометой на поле 36:8.",
            },
            {
                source: "bible_verses",
                file: "bible-verses-la-vulgata",
                edition: "la-vulgata",
                title: "Вульгата Климентины, 1592 — текст стихов",
                license: PUBLIC_DOMAIN,
                attribution:
                    "Текст: Biblia Sacra Vulgata (Clementina, 1592), оцифровка "
                    + "github.com/lb42/vulgClem. Разбор на книги и согласование "
                    + "нумераций — проект «Уставные чтения» "
                    + `(${SITE_HOST}), CC BY 4.0`,
                note:
                    "Условий оцифровщик не заявляет; основание то же, что у "
                    + "церковнославянской и грузинской. Латинский канон уже нашего: "
                    + "3-й Маккавейской в нём нет вовсе, и это свойство традиции, а не "
                    + "недостача выгрузки.",
            },
            {
                source: "bible_verses",
                file: "bible-verses-en-brenton",
                edition: "en-brenton-kjv",
                testament: "ot",
                title: "Брентон, 1851 — текст стихов Ветхого Завета по Септуагинте",
                license: PUBLIC_DOMAIN,
                attribution:
                    "Текст: Translation of the Greek Septuagint into English by Sir "
                    + "Lancelot C. L. Brenton (1851), ebible.org/eng-Brenton — "
                    + "общественное достояние. Разбор на книги и согласование "
                    + "нумераций — проект «Уставные чтения» "
                    + `(${SITE_HOST}), CC BY 4.0`,
                note:
                    "Половина пары: английское издание собрано из двух, как и греческое. "
                    + "Брентон взят на Ветхий Завет потому, что переведён с Септуагинты — "
                    + "KJV и ASV оба с еврейского, а паремии читаются по греческому. "
                    + "Псалтирь у него греческого счёта: 151 псалом, 9-й в 39 стихов.",
            },
            {
                source: "bible_verses",
                file: "bible-verses-en-kjv",
                edition: "en-brenton-kjv",
                testament: "nt",
                title: "King James Version, 1611 — текст стихов Нового Завета",
                license: PUBLIC_DOMAIN,
                attribution:
                    "Текст: King James Version (набор 1769), ebible.org/eng-kjv — "
                    + "общественное достояние. Разбор на книги и согласование "
                    + "нумераций — проект «Уставные чтения» "
                    + `(${SITE_HOST}), CC BY 4.0`,
                note:
                    "Вторая половина пары. Textus Receptus византийского типа, и это то, "
                    + "что читается за английской службой. ASV византийского текста был "
                    + "отведён сверкой: Патриаршее издание печатает 1 Ин. 5:7 и Деян. 8:37, "
                    + "отбивая скобками, а тот выбрасывает оба — вышли бы две дыры там, "
                    + "где греческая строка есть.",
            },
            {
                source: "bible_verses",
                file: "bible-verses-ka-mcxeta",
                edition: "ka-mcxeta",
                title: "Грузинская Библия древнегрузинским — текст стихов",
                license: PUBLIC_DOMAIN,
                attribution:
                    "Текст: ძველი ქართულით, orthodoxy.ge/tserili/biblia_sruli — "
                    + "Ветхий Завет по Мцхетской рукописи, Новый преподобных Евфимия и "
                    + "Георгия Святогорцев. Разбор на книги, стиховая разбивка и "
                    + "согласование нумераций — проект «Уставные чтения» "
                    + `(${SITE_HOST}), CC BY 4.0`,
                note:
                    "Условий у этой оцифровки НЕ ЗАЯВЛЕНО, и это надо знать, беря файл. "
                    + "Сами памятники в общественном достоянии по возрасту: Мцхетская "
                    + "рукопись XVII–XVIII века, перевод Святогорцев — XI. Тот же текст "
                    + "выложен на archive.org (ogb_20220326) под Public Domain Mark, но "
                    + "метку там ставил не оцифровщик, и полагаться на неё как на "
                    + "разрешение нельзя. Отдаём потому, что верная передача текста, "
                    + "свободного от притязаний, своего права не порождает. "
                    + "Церковнославянская и латинская копии в выгрузку по-прежнему не "
                    + "идут — у них к тому же есть живые проекты-оцифровщики, чьи "
                    + "условия спрашивать надо у них.",
            },
            {
                source: "bible_verses",
                file: "bible-verses-grc-nt",
                edition: "grc-lxx-pat",
                testament: "nt",
                title: "Патриаршее издание 1904/1912 — текст стихов Нового Завета",
                license: PUBLIC_DOMAIN,
                attribution:
                    "Текст: Η Καινή Διαθήκη, 1904 Patriarchal Greek New Testament, "
                    + "ebible.org/find/details.php?id=grcbyz — общественное достояние. "
                    + "Разбор на книги и согласование нумераций — проект «Уставные чтения» "
                    + `(${SITE_HOST}), CC BY 4.0`,
                note:
                    "Общественное достояние, а не лицензия слоя: требовать указания "
                    + "источника на текст, который свободен от притязаний, мы не вправе. "
                    + "Наша работа над ним — разбор и согласование — под CC BY 4.0.",
            },
            {
                source: "bible_verses",
                file: "bible-verses-grc-ot",
                edition: "grc-lxx-pat",
                testament: "ot",
                title: "Септуагинта (Свит, Кембридж 1909–1930) — текст стихов",
                license: GPL3,
                attribution:
                    "Оцифровка: LXX-Swete-1930 (github.com/eliranwong/LXX-Swete-1930), GPL-3.0. "
                    + "Разбор на книги и согласование нумераций — проект «Уставные чтения» "
                    + `(${SITE_HOST}), CC BY 4.0`,
                note:
                    "ТОЛЬКО этот файл идёт под GPL-3.0, а не весь слой: сам текст — чужая "
                    + "оцифровка, полный текст лицензии лежит рядом в GPL-3.0.txt. Издание "
                    + "Свита в общественном достоянии по возрасту, условия ставит оцифровщик. "
                    + "Новый Завет этого же издания в выгрузку не входит.",
            },
        ],
        pointers: [
            {
                what: "Чувашская Библия — оба издания, гражданской и церковной азбукой",
                why: "Перевод живой и под правом Института перевода Библии; возрастом он "
                    + "не освобождён, и рассуждение о верной передаче старого текста к нему "
                    + "не относится.",
                where: "ibt.org.ru — условия спрашивать там.",
            },
            {
                what: "Псалтирь, Евангелие и Апостол дониконовского извода (cs-old-rite)",
                why: "Текст набран не нами: его выложил старообрядческий сайт «Уставщик», и "
                    + "он прямо заявляет © в подвале. Это НЕ молчание, а притязание, и "
                    + "рассуждение о верной передаче на него не распространяется — оно "
                    + "разбирает случай, когда оцифровщик не сказал ничего. Наша доля "
                    + "здесь — раскодировка шрифтовой раскладки UCS, разбор и СТИХОВОЕ "
                    + "ДЕЛЕНИЕ: книги стихов не нумеруют, и деление наведено сличением с "
                    + "Елизаветинской. Эта доля в выгрузке ЕСТЬ — 8 803 адреса в "
                    + "bible-concordance, без текста.",
                where: "ustavshik.ru — условия спрашивать там.",
            },
        ],
    },
    {
        id: "temples",
        title: "Храмы и престолы",
        license: ODBL,
        attribution:
            "© участники OpenStreetMap (ODbL); данные Wikidata (CC0); "
            + `разметка престолов — проект «Уставные чтения» (${SITE_HOST})`,
        rationale:
            "Каталог выведен из OpenStreetMap (53 091 запись) и Wikidata (11 763). OSM "
            + "распространяется под ODbL: она требует указания источника и держит производные "
            + "базы на тех же условиях. Поэтому слой отдельный и НЕ под CC BY 4.0 — иначе мы "
            + "пересдавали бы чужую базу на своих условиях. Наша работа здесь — разбор "
            + "посвящений (какому святому престол), привязка к уставу и сведение с каталогом "
            + "святых; она отдаётся вместе со слоем на условиях слоя.",
        collections: [
            {
                source: "temples",
                file: "temples",
                title: "Храмы с престолами",
                note: "Присоединяется к dedications и saints из слоя corpus по их идентификаторам.",
            },
        ],
    },
];

/**
 * Коллекции, которые наружу не идут, и почему. Список закрытый: он же служит
 * доказательством, что о каждой таблице базы подумали.
 */
export const EXCLUDED: Record<string, string> = {
    dneslov_names: "снимок чужих святцев (dneslov.org), условия правообладателем не заявлены",
    dneslov_memories: "то же",
    notes: "рабочие заметки набора",
    channelPosts: "очередь автопостинга, служебное",
    mentionCandidates: "черновики разметки упоминаний, не выверены",
    saint_proposals:
        "рабочая очередь предложений святых к добавлению: черновики со статусами "
        + "и разбором дублей, не выверены человеком — то же, что mentionCandidates",
    // Приходское (parishSettings, parishEdits, parishSchedules, templeClaims,
    // templeAdmins) в этой базе больше не лежит — переехало в typikon-users
    // (см. src/lib/parish/db.ts), и выгрузка до него не достаёт. Записи
    // оставлены на случай, если где-то ещё жива непереехавшая копия базы:
    // сборка не должна на ней спотыкаться, а выложить эти данные — тем более.
    parishEdits: "правки приходского расписания — чужие данные приходов, живут в typikon-users",
    parishSchedules: "то же",
    parishSettings: "то же",
    templeAdmins: "кто ведёт расписание храма — персональные данные, живут в typikon-users",
    templeClaims: "заявки на ведение — персональные данные, живут в typikon-users",
    dates: "пустая коллекция, оставшаяся от прежней модели",
    openbible_verses:
        "сырой перечень стихов OpenBible в ЕГО нумерации (OSIS). Наш вклад здесь — перевод "
        + "в славянскую нумерацию сверкой по тексту, и он отдаётся отдельно, в place-mentions. "
        + "Сам перечень — чужой набор данных целиком (CC BY 4.0), и пересдавать его копией "
        + "незачем: он берётся у источника, https://www.openbible.info/geo/",
    place_nikifor_candidates:
        "черновики отождествления мест со статьями энциклопедии Никифора, на ревью — "
        + "то же, что mentionCandidates: пока не просмотрено человеком, наружу не идёт",
    health_snapshots:
        "снимки панели здоровья — счёт наших собственных недоделок во времени. "
        + "Это операционная кухня, а не корпус: числа в ней говорят о нас, а не о "
        + "памятниках, и снаружи были бы шумом. Сама панель открыта правом content.",
};

/** Коллекции, которые уходят наружу хотя бы одним файлом. */
export const exportedCollections = (): Set<string> => {
    const names = new Set<string>();
    LAYERS.forEach((layer) => layer.collections.forEach((collection) => {
        if (collection.source) names.add(collection.source);
    }));
    return names;
};

/**
 * Коллекции базы, о которых никто ничего не сказал. Сборка на этом останавливается:
 * молчание не значит «можно выкладывать».
 */
export const unclassified = (collectionNames: string[]): string[] => {
    const exported = exportedCollections();
    return collectionNames
        .filter((name) => !exported.has(name) && !(name in EXCLUDED))
        .sort();
};

/**
 * Приведение документа к простому JSON: ObjectId — строкой, дата — по ISO,
 * ключи по алфавиту. Сортировка ключей нужна не для красоты: без неё две сборки
 * на одной базе давали бы разные байты, и сверить выгрузки было бы нечем.
 */
export const normalize = (value: any): any => {
    if (value === null || value === undefined) return null;

    if (typeof value?.toHexString === "function") return value.toHexString();
    if (value instanceof Date) return value.toISOString();
    if (Array.isArray(value)) return value.map(normalize);

    if (typeof value === "object") {
        const out: Record<string, any> = {};
        Object.keys(value).sort().forEach((key) => {
            out[key] = normalize(value[key]);
        });
        return out;
    }

    return value;
};

/** Документ наружу: снятые поля и приведение к простому JSON. */
export const prepare = (doc: Record<string, any>, drop?: Record<string, string>): any => {
    const copy: Record<string, any> = {};
    Object.keys(doc).forEach((key) => {
        if (drop && key in drop) return;
        copy[key] = doc[key];
    });
    return normalize(copy);
};
