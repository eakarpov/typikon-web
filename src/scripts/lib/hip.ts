...
const cleanCommon = (input: string, stats: Stats): string => {
    ...
    // @ обрамляет слово, к которому в книге дан перевод сноской: проверено — все
    // девять мест в Ифике стоят при сноске-глоссе, ни одного исключения. Само
    // выделение не переносим: открывающий маркер сдвинут на букву-две внутрь слова,
    // и курсив пришёлся бы на «рохлажденіе» вместо «прохлажденіе». Смысл несёт сноска.
    s = swap(s, stats, "глоссы", /\\?@/g, "");
...
// --- Исходный восьмибитный HIP -------------------------------------------
//
// Orthlib отдаёт тексты уже наполовину переведёнными в юникод, а в исходном
// формате надстрочные знаки записаны ASCII-символами, буквы — своими вариантами
// через подчёркивание, а файл лежит в CP1251. Эта ступень приводит такой текст
// к тому же виду, после чего работает общая чистка.
//
// Таблица собрана по данным, а не по описанию формата: 302 тыс. знаков «Алфавита
// духовного» дали 21 415 ударений, 8146 звательцев, 5150 варий, 1588 титл.
const HIP8_LETTERS: [RegExp, string][] = [
    // Диграфы числовых букв — раньше одиночных, иначе «_Пс» разберётся как «_П».
    [/_[Пп][Сс]/g, "ѱ"], [/_[Кк][Сс]/g, "ѯ"],
    // Варианты начертаний: широкое е, ук, широкое о, і, омега.
    [/_е/g, "є"], [/_Е/g, "Є"], [/_у/g, "ѹ"], [/_У/g, "Ѹ"],
    [/_о/g, "ѻ"], [/_О/g, "Ѻ"], [/_i/g, "і"], [/_I/g, "І"],
    [/_w/g, "ѡ"], [/_W/g, "Ѡ"],
    // ять и юсовые: в этом источнике строчная j, у orthlib — заглавная.
    [/jь/g, "ѣ"], [/[Jj]Ь/g, "Ѣ"], [/Jь/g, "Ѣ"],
    [/ja/g, "ꙗ"], [/jа/g, "ꙗ"], [/Jа/g, "Ꙗ"],
    [/(?<![A-Za-z])s(?![A-Za-z])/g, "ѕ"],
    // Ижица: с кендемой и без.
    [/v"/g, "ѷ"], [/V"/g, "Ѷ"], [/v/g, "ѵ"], [/V/g, "Ѵ"],
];

export const hip8ToUnicode = (raw: string, stats: Stats): string => {
    // Файлы пришли из-под Windows: без этого перевод каретки не даёт распознать
    // границу абзаца, и вся глава склеивается в один абзац.
    let s = raw.replace(/\r\n?/g, "\n");
    for (const [re, to] of HIP8_LETTERS) s = swap(s, stats, "буквы", re, to);
    // Выносное ъ — это ерок; общая ступень ниже разбирает остальные выносные.
    s = swap(s, stats, "выносные", /\\ъ/g, "̾");
    s = swap(s, stats, "ударения", /'/g, OXIA);
    s = swap(s, stats, "звательца", /=/g, "҆");
    s = swap(s, stats, "титла", /~/g, TITLO);
    return s;
};

export const normalizeHip = (
    raw: string,
    options: { hip8?: boolean } = {},
): { content: string; footnotes: string[]; dropped: string[]; stats: Stats } => {
    const stats: Stats = {};
    let s = options.hip8 ? hip8ToUnicode(raw, stats) : raw;
