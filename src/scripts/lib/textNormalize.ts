...
// Нормализация ЦС живёт в @/utils/churchSlavonic — ею пользуется и приложение
// (полнотекстовый поиск), а не только скрипты. Реэкспорт, чтобы не переписывать импорты.
export { normalizeChurchSlavonic } from "@/utils/churchSlavonic";
