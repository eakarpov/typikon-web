...
const SPECS: Spec[] = [
    ...
    { db: "typikon-users", collection: "users", key: { "auth.vk.userId": 1 }, options: { sparse: true },
      why: "вход через VK отключён; индекс оставлен, чтобы найти записи, у которых другой привязки нет" },
    { db: "typikon-users", collection: "users", key: { "auth.telegram.userId": 1 }, options: { sparse: true },
      why: "вход через Telegram" },
    { db: "typikon-users", collection: "users", key: { "auth.yandex.userId": 1 }, options: { sparse: true },
      why: "вход через Яндекс" },
