// Выгружает подтверждённые связи «память книги — святой» в правила typikon-rules.
//
...
// пропала бы наутро. Поэтому подтверждённое едет в `rules/memory_saints.yaml`,
// то есть во ВХОД сборки, а проставляет dneslov_id уже migrate_memory_saints.py.
//
// Тот же порядок, что у остальных ручных сведений корпуса: правится вход,
// пересобирается база (см. шапку build_db.py).
//
// Запуск:  npm run export:memory-saints  [-- --out <путь>]
import "@/scripts/lib/env";
import fs from "node:fs";
import path from "node:path";
import clientPromise from "@/lib/mongodb";

const DEFAULT_OUT = path.resolve(process.cwd(), "..", "typikon-rules", "rules", "memory_saints.yaml");
...
const main = async () => {
    ...
    const col = client.db("typikon").collection("memory_saint_links");
    ...
    const lines = [
        "# Связи памятей книг со святыми: memories.dneslov_id.",
        "#",
        "# Файл СОБРАН, а не написан: его выгружает",
        "# typikon-web/src/scripts/export-memory-saints.ts из подтверждённых в админке",
        "# связей (/admin/akathists?target=memory). Править руками можно, но следующая",
        "# выгрузка перезапишет — правьте решение в админке.",
        "#",
        "# Читает его src/migrate_memory_saints.py при сборке корпуса.",
        `# Выгружено: ${rows.length} подтверждённых; ждут решения ${pending}, отклонено ${rejected}.`,
        "",
        "links:",
        ...rows.map((r: any) =>
            `  - memory_id: ${quote(r.memoryId)}\n`
