...
//
// ЗАПИСЬ — В NFC, как и всё издание (см. normalize-bible-unicode.ts). Прежде
// половины греческого расходились: Ветхий Завет был записан знаками «с оксией» из
// Greek Extended, Новый — в NFC, и первая правка топаза дважды на этом споткнулась.
//
...
const main = async () => {
    ...
    for (const fix of CORRECTIONS) {
        ...
        // Издание держится в NFC (npm run bible:normalize), и правка пишется в нём же.
        // Сравнение побайтовое, а не по NFC: правка считается стоящей, только если
        // стих написан ТЕМИ ЖЕ знаками, что и всё издание. Иначе — чиним и знаки.
        const target = fix.to.normalize("NFC");
