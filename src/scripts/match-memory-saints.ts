...
import {
    cachedSearch, judge, queryTerms, stem, wordsOf, type Verdict,
...
const STOP = new Set([
    ...
    "архиепископа", "епископа", "епископов", "архиепископов", "митрополитов",
    "святителей", "преподобномучеников", "мучениц", "исповедниц",
    "митрополита", "патриарха", "игумена",
...
const main = async () => {
    ...
    for (const row of rows) {
        ...
        // эпитета, и верный ответ есть только в объединении. Какие три —
        // решает избирательность, а не порядок в заголовке (см. queryTerms).
        const terms = queryTerms(ours);
