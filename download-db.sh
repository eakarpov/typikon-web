...
sshpass -f <(printf '%s\n' $PASSWORD) scp download-remote-db.sh $USERNAME@$HOST:/var/www/typikon.su/typikon-web/download-remote-db.sh

rm -rf db-download

sshpass -f <(printf '%s\n' $PASSWORD) ssh $USERNAME@$HOST 'bash -s' < download-remote-db.sh

sshpass -f <(printf '%s\n' $PASSWORD) scp $USERNAME@$HOST:/var/www/typikon.su/typikon-web/db-download.zip db-download.zip
