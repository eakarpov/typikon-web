...
# ПРИХОДСКОГО И ПОСТОВ КАНАЛА ЗДЕСЬ НЕТ — и это нарочно.
#
...
#
# Посты канала (`channelPosts`) уехали туда же и по той же причине: их готовит
# крон НА ПРОДЕ, там же их правят и подтверждают, а здешняя копия — вчерашний
# слепок. Каждая выкладка корпуса стирала подготовленные черновики, и до своего
# часа они не доживали (см. src/lib/channelPosts/db.ts).
