...
#
# Путь к файлу берётся из RULES_DB в файле окружения цели: если он переедет,
# менять придётся в одном месте, а не в двух. У испытательной машины файл свой,
# и корпус на ней может лежать по своему пути.
#
# Цель — ключом (release-target.sh):
#     npm run release:rules-db
#     npm run release:rules-db -- --target test

. "$(dirname "$0")/release-target.sh"

RULES_DB_LOCAL=${RULES_DB_LOCAL:-../typikon-rules/src/data.db}
RULES_DB_REMOTE=$(env_value RULES_DB)

if [ -z "$RULES_DB_REMOTE" ]; then
    echo "в $ENV_FILE нет RULES_DB — некуда класть корпус"
    exit 1
fi
...
scp_put rules-db.zip "rules-db.zip"

ssh_run "RULES_DB_REMOTE='$RULES_DB_REMOTE' bash -s" < "$RELEASE_DIR/rules-db-remote.sh"
