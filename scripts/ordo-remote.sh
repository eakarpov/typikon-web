...
# ОТВЕЧАТЬ МАЛО — НАДО СОБИРАТЬ. Список канв служба отдаёт, даже когда ей не
# хватает файлов для самой сборки: так было с spec/registry, и выкладка
# рапортовала успех, а на сайте не собиралась половина служб. Собираем одну
# настоящую службу на сегодня и показываем ошибку, если она есть.
TODAY=$(date +%F)
SVC=$(curl -s -m 60 "http://127.0.0.1:8767/sutki?date=$TODAY&service=vespers&service=liturgy")
# ищем ПОЛЕ ошибки, а не слово: у шагов бывает "kind":"error", и это не сбой
if [ -z "$SVC" ] || printf '%s' "$SVC" | grep -qE '"error": ?"'; then
    echo "служба устава отвечает, но НЕ СОБИРАЕТ:"
    printf '%s' "$SVC" | grep -oE '"error": ?"[^"]*"' | sort -u
    exit 1
fi
echo "служба устава собирает"
